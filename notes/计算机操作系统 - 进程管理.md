...
> 饥饿：一个进程长期处于等待状态而得不到运行
>
> 饿死：一个进程永久性的等待，或者是等待时间过长即使任务完成也没有意义
>
> 饿死 vs 死锁：
>
> 1. 饿死可以只有一个进程；死锁必须有两个或两个以上进程才能造成死锁
> 2. 饿死的进程可以是就绪状态（比如短作业优先调度，进程在就绪队列一直得不到运行）；死锁一定是阻塞状态（waiting）
...
> 哲学家进餐问题有多种解法：
>
> 1. 规定奇数位哲学家先拿起左手筷子，偶数位哲学家先拿起右手筷子，这样至少有一位哲学家不需要参与筷子的竞争。
>
> 2. 只有在左右两个邻居都没有进餐的情况下可以进餐，同时必须同时拿起左右两根筷子（原子操作）
>
>
>    下面的代码可以不用看了，写的有点繁琐。
...
> 一个整形信号量count记录读进程的数量，一个互斥信号量mutex对读写加锁。
>
> 读进程读之前需要判断mutex是否加锁，无锁count++，开始读操作
>
> 写进程写之前判断count是否为0，为0则mutex加锁，不为0则阻塞等待
>
> 
>
> 这小节下面的文本可以不需要看了。
...
- 订阅机制：读进程可以根据消息类型有选择地接收消息，而不像 FIFO 那样只能默认地接收。
